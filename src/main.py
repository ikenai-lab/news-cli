...
async def async_main(model: str, limit: int):
    ...
    console.print(
        f"[bold green]News Agent Ready! (Model: {model}, Limit: {limit})[/bold green]"
    )
    ...
    # Populate cache
    _populate_cache(agent, articles)

    console.print(
        f"[dim]📰 {len(articles)} articles loaded. Use /read <id> (e.g., 1, 2) to read.[/dim]\n"
    )
    ...
    console.print(
        "[dim]Type '/' to see available commands, or just type your question[/dim]\n"
    )
    ...
    while True:
        try:
            ...
            if response == "REFRESH_BRIEFING":
                ...
                articles = await render_briefing(
                    categories, article_limit=min(limit, 5)
                )
                _populate_cache(agent, articles)
...
def _populate_cache(agent, articles):
    """Helper to populate agent cache with articles."""
    agent.search_cache = {}
    agent.id_map = {}
...
def main(
    ...
    model: str = typer.Option(
        app_config.default_model, help="Ollama model to use for the agent"
    ),
    limit: int = typer.Option(
        app_config.default_limit,
        min=1,
        max=20,
        help="Number of articles per search (1-20)",
    ),
):
    """
