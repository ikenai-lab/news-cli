...
async def search_news(query: str, max_results: int = 5, timelimit: str | None = None) -> list[dict]:
    ...
    try:
        ...
        if news_results:
            for result in news_results:
                ...
                # Strict Filters
                if "msn.com" in url.lower() or "msn" in source: continue
    ...
    if len(results) < max_results:
        ...
        try:
            ...
            text_results = await asyncio.to_thread(_search_text_sync, query, remaining * 2, timelimit)
            if text_results:
                for result in text_results:
                    url = result.get("href", "")
                    
                    # Strict Filters
                    if "msn.com" in url.lower(): continue
                    if url in seen_urls: continue
